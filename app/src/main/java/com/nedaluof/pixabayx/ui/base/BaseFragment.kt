...
abstract class BaseFragment<DB : ViewDataBinding>(
  ...
  fun <T> StateFlow<T>.collectStateFlow(block: suspend (T) -> Unit): Job =
    lifecycleScope.launch {
      this@collectStateFlow.collect { block(it) }
    }

  fun <T> Flow<T>.collectFlow(block: suspend (T) -> Unit): Job =
    lifecycleScope.launch {
